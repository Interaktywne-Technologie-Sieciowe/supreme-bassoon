...
const reset = async () => {
  ...
  try {
    await fetch('http://localhost:3000/api/users/resetPassword', {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(err.message || 'Wystąpił błąd')
    }

    success.value = 'Link do resetowania hasła został wysłany na podany adres e-mail.'
    email.value = ''
  } catch (err) {
